 ...
 * and call view.
 */
public class McuActivity extends Activity implements AppRTCClient.SignalingEvents,
...
//    requestWindowFeature(Window.FEATURE_NO_TITLE);
    ...
    if (loopback || !DirectRTCClient.IP_PATTERN.matcher(roomId).matches()) {
      appRtcClient = new McuRTCClient(this);
    }
    ...
    peerConnectionClient = new PeerConnectionClient(
        getApplicationContext(), eglBase, peerConnectionParameters, McuActivity.this);
...
//    mediaPlay();
  ...
  public boolean playWavInCall() {

    int i = 0;
    byte[] music = null;
    InputStream is = getApplicationContext().getResources().openRawResource(R.raw.ring);

    AudioTrack audioTrack = new AudioTrack(AudioManager.STREAM_MUSIC, 11000,
      AudioFormat.CHANNEL_CONFIGURATION_MONO, AudioFormat.ENCODING_PCM_16BIT,
      1024*1024, AudioTrack.MODE_STREAM);
  ...
  private void playSound(final int soundID) {
    final int ringId = R.raw.ring;
    playingThread = new Thread(new Runnable() {
      public void run() {
        int minBufferSize = AudioTrack.getMinBufferSize(11000, AudioFormat.CHANNEL_OUT_MONO, AudioFormat.ENCODING_PCM_16BIT);
        AudioTrack audioTrack = new AudioTrack(AudioManager.STREAM_MUSIC, 11000,
          AudioFormat.CHANNEL_CONFIGURATION_MONO, AudioFormat.ENCODING_PCM_16BIT,
          500*minBufferSize, AudioTrack.MODE_STREAM);
        audioTrack.play();
        int i = 0;
        byte [] buffer = new byte[minBufferSize];
        InputStream inputStream = getResources().openRawResource(ringId);
        try {
          inputStream.read(new byte[44]);
          while((i = inputStream.read(buffer)) != -1) audioTrack.write(buffer, 0, i);
          inputStream.close();
        } catch(Exception e) {
          Log.e(TAG, "AudioTrack playSound err:" + e.toString());
        }
      }
